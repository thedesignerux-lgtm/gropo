...
// Checkout "Unirme al gropo". Modelo de PROYECCION: el precio del resumen, el
// banner, el subtotal y el boton reaccionan a [unidades actuales] + [qty]. El
...
// de precios es publica). La mecanica bancaria NO se expone: el front es ciego al
// importe retenido -- solo ve el precio de producto.
//
// Presentacion alineada con el diseno de ficha (cabecera producto, cantidad,
// tarjeta de ahorro potencial, progreso "X ciclistas", tarjetas de confianza,
// logos de pago y acordeon). La LOGICA de precio/cantidad/direccion/Stripe/holds
// es la misma de siempre; aqui solo cambia el marcado.
...
/**
 * Un escalon de la curva publica de precios.
 *
 * `demand` es la demanda EFECTIVA a ESE precio (`tier_demand.effective_demand`):
 * cuantas unidades lo comprarian. No es un numero global -- un esperador con
 * objetivo 150 EUR cuenta en el tramo de 150 EUR y en los mas baratos, pero no en el
 * de 200 EUR. Ver P2-01b en `KNOWN_ISSUES.md`.
 */
...
export type JoinGroup = {
  ...
  /** El vendedor declaro que los precios ya incluyen el envio (UX-03). */
  shipping_included: boolean;
  tiers: Tier[];
};

/**
 * Stock restante de la puja que da el mejor precio. `null` = sin limite conocido.
 *
 * P2-01 . Antes esto restaba `total_units`, que es la demanda EFECTIVA al precio
 * actual: deja fuera a los esperadores que apuntan a un tramo mas barato. Como
 * esos esperadores SI ocupan plaza, el resultado sobreestimaba lo disponible y
 * el selector dejaba pedir unidades que ya no existian; el rechazo llegaba
 * despues, al pagar. Ahora resta `committed_units`, que es exactamente la misma
 * suma que usa `prepare_join` para aceptar o rechazar en servidor.
 *
 * Sigue siendo una foto del momento en que se pinto la pagina: entre eso y el
...
/**
 * A-12 . El stock servia solo para IMPEDIR, nunca para AVISAR.
 *
 * `remainingStock` ya calculaba bien las unidades libres y el selector topaba ahi,
 * pero el badge decia "En stock" exactamente igual con 2 unidades que con 200. Y
 * el peor sitio para callarlo es un grupo que ya tiene desbloqueado su mejor precio:
 * ahi no queda ninguna palanca de precio y la unica razon para decidir hoy es que se
 * acaban -- justo lo que no se decia.
 *
 * Umbral en 5 unidades: por debajo de eso el numero comunica algo ("quedan 2");
 * por encima, un numero grande no aporta y ademas revela el inventario del vendedor
 * sin necesidad.
 */
...
// 549 EUR . 62,50 EUR -- sin decimales si es entero, coma decimal y EUR al final
const eur = (n: number) =>
  (Number.isInteger(n) ? String(n) : n.toFixed(2).replace('.', ',')) + ' €';

/* Ventana de urgencia: solo se muestra cuenta atras si el cierre esta cerca.
   Un contador de "155d" mata el FOMO e invita a procrastinar, asi que por
   encima de este umbral se ensena solo el dia de cierre. */
const URGENCY_WINDOW_DAYS = 14;

/* Countdown compacto del checkout: "2d 14h" en morado (rojo el ultimo dia).
...
function CompactCountdown({ closesAt }: { closesAt: string }) {
  ...
  if (!state) return <span className="text-lg font-extrabold text-neutral-300">&mdash;</span>;
  return (
    <span
 ...
 * `autoComplete`. Tres consecuencias, de mas a menos cara:
 *
 *  1. El autorrelleno del movil NO FUNCIONABA. Ocho campos tecleados a mano en una
 *     pantalla pequena, en el punto de maxima friccion del embudo. De todo lo que
 *     encontro la auditoria, es lo que mas cuesta en conversion.
 *  2. Al escribir, la etiqueta desaparece (el defecto clasico del placeholder-como-
 ...
 *
 * La etiqueta va en `sr-only`: arregla 1 y 3 sin tocar el diseno aprobado. Hacerla
 * visible resolveria tambien 2, pero cambia la altura del formulario y eso es
 * decision visual de Benjamin, no mia.
 */
function Field({
...
export default function JoinFlow({
  ...
  // Tope: 10 por comprador y nunca mas del stock restante de la puja ganadora.
  // El Math.max(1, ...) de antes hacia que con stock 0 el selector siguiera
  ...
  // servidor, que es la autoridad; la escalera de aqui abajo solo pinta.
  useEffect(() => {
  ...
  // -- PROYECCION (reactiva a total_units + qty) --
  ...
  //
  // Antes todo esto se media contra `group.total_units`, un solo numero: la
  // demanda al precio vigente. Con eso, un grupo al que le falta 1 unidad para
  // bajar a 150 EUR podia anunciar "faltan 8", porque los esperadores que apuntan
  // a 150 EUR no entran en la demanda a 200 EUR. La caja de precio (que si viene del
  // servidor) decia 150 EUR y el progreso, justo debajo, decia que faltaban 8.
  //
  // Cada tramo trae ahora su propia demanda. `withYou` le suma la compra en
  // curso SOLO si contaria en ese tramo: quien compra ahora cuenta en todos;
  // quien espera a un objetivo cuenta en el suyo y en los mas baratos, igual
  ...
  // Hold para Stripe: siempre target x qty para esperadores (techo de seguridad)
  ...
  const elementsOptions = useMemo(
    () => ({
      ...
      // rechaza la confirmacion en modo diferido con "setup_future_usage mismatch".
  ...
  // -- Stepper de tramos (diseno 4b): estados y relleno de la barra --
  // La barra refleja las unidades PROYECTADAS (grupo + las que elige el usuario),
  // en coherencia con el precio, que tambien es proyectado.
  ...
  // Posicion CONTINUA en la barra. El relleno de un segmento es lo llena que
  // esta la exigencia del tramo que viene: su demanda dividida entre sus
  ...
  return (
    /* A-32 . En escritorio esto es una rejilla de dos columnas: el formulario a la
       ...
       vez. En movil no hay rejilla y el orden del DOM manda: producto y luego
       formulario, que es como estaba. La logica de dinero no se toca -- esto es
       colocacion, nada mas. */
    <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_368px] lg:items-start lg:gap-8">
      {/* Columna DERECHA en escritorio, bloque de arriba en movil. */}
      <div className="lg:col-start-2 lg:row-start-1 lg:min-w-0">
      {visualMode === 'esperar' ? (
        <>
          {/* ====== Design B -- modo esperar ====== */}

          {/* Header */}
          <section className="px-4 pt-4">
            <h2 className="text-lg font-bold text-neutral-900">Únete al grupo</h2>
          </section>
            ...
            <div className="rounded-[18px] border border-black/[0.08] bg-white p-[15px]">
              ...
              <div className="mt-[13px] flex items-center gap-2">
                <div className="flex h-[34px] items-center gap-2.5 rounded-[11px] border border-black/[0.08] bg-[#F2F7F7] px-1.5">
                  <button type="button" onClick={() => setQuantity((q) => Math.max(1, q - 1))} disabled={quantity <= 1 || checkoutBusy} aria-label="Quitar una unidad" className="grid h-6 w-6 place-items-center text-[17px] leading-none text-neutral-600 disabled:text-neutral-300">&minus;</button>
          ...
          {/* Orange badge: price not yet unlocked */}
          <section className="px-4 pt-3.5">
            <div className="flex items-center gap-3 rounded-xl bg-[#FEF3E2] px-4 py-3">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#B24A00" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg>
              <span className="text-sm font-semibold text-[#B24A00]">Este precio aún no está desbloqueado</span>
            </div>
          </section>
            ...
            <div className="rounded-[18px] border border-black/[0.08] bg-white p-[15px]">
              <div className="flex items-baseline justify-between">
                <span className="text-sm font-semibold text-neutral-900">
                  {targetIdx >= 0 ? ladder[targetIdx].withYou : 0} de {targetIdx >= 0 ? ladder[targetIdx].minUnits : 0} compradores
                </span>
                <span className="text-xs font-semibold text-neutral-500">
                  {targetIdx >= 0 && ladder[targetIdx].minUnits > 0
                    ? `${Math.min(100, Math.round((ladder[targetIdx].withYou / ladder[targetIdx].minUnits) * 100))}%`
                    : '0%'}
                </span>
              </div>
              <div className="mt-2 h-2.5 overflow-hidden rounded-full bg-neutral-100">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-[#F59E0B] to-[#D97706] transition-[width] duration-300 ease-out"
                  style={{
                    width: `${targetIdx >= 0 && ladder[targetIdx].minUnits > 0
                      ? Math.min(100, (ladder[targetIdx].withYou / ladder[targetIdx].minUnits) * 100)
                      : 0}%`,
                  }}
                />
              </div>
              <div className="mt-2.5 flex items-center gap-1 text-xs font-semibold text-neutral-500">
          ...
          {/* Target price card */}
          <section className="px-4 pt-3.5">
            <div className="rounded-[18px] border border-dashed border-[#B24A00]/40 bg-[#FFFBF5] p-4">
              <div className="text-xs font-semibold text-neutral-500">Precio objetivo</div>
              <div className="mt-1 text-[28px] font-extrabold leading-none tracking-tight text-[#B24A00]">{eur(efectiveTargetPrice)}</div>
              {savingsPerUnit > 0.01 && (
                <div className="mt-2 inline-flex items-center rounded-lg bg-[#E6F5EC] px-2.5 py-1 text-xs font-bold text-[#0B7B44]">
                  Ahorras {fmtSaving(savingsPerUnit * quantity)}
                </div>
              )}
            </div>
          </section>
          ...
          <section className="px-4 pt-3.5">
            <div className="flex items-center justify-between rounded-xl bg-neutral-50 px-4 py-3">
              <span className="text-sm text-neutral-600">Total hoy (retención temporal)</span>
              <span className="text-lg font-bold text-neutral-900">0 €</span>
            </div>
          </section>

          {/* "Que pasa ahora?" explainer */}
          <section className="px-4 pt-3.5">
            <div className="rounded-[18px] border border-black/[0.08] bg-white p-[15px]">
              <div className="text-[15px] font-extrabold text-neutral-900 mb-3">¿Qué pasa ahora?</div>
              <div className="flex flex-col gap-4">
                <div className="flex gap-3">
                  <div className="grid h-6 w-6 flex-none place-items-center rounded-full bg-brand text-xs font-bold text-white">1</div>
                  <div>
                    <div className="text-sm font-bold text-neutral-900">Te unes al grupo</div>
                    <div className="mt-0.5 text-xs leading-relaxed text-neutral-500">Tu reserva ayuda a alcanzar el volumen necesario para el descuento.</div>
                  </div>
                </div>
                <div className="flex gap-3">
                  <div className="grid h-6 w-6 flex-none place-items-center rounded-full bg-brand text-xs font-bold text-white">2</div>
                  <div>
                    <div className="text-sm font-bold text-neutral-900">El grupo crece</div>
                    <div className="mt-0.5 text-xs leading-relaxed text-neutral-500">Cuantos más se unen, más baja el precio. Te avisamos de cada avance.</div>
                  </div>
                </div>
                <div className="flex gap-3">
                  <div className="grid h-6 w-6 flex-none place-items-center rounded-full bg-brand text-xs font-bold text-white">3</div>
                  <div>
                    <div className="text-sm font-bold text-neutral-900">Se alcanza tu precio</div>
                    <div className="mt-0.5 text-xs leading-relaxed text-neutral-500">Solo se cobra si el grupo llega a tu objetivo. Si no, se cancela sin coste.</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <>
          {/* ====== Design A -- modo comprar (includes targetReached) ====== */}

          {/* Header */}
          <section className="px-4 pt-4">
            <h2 className="text-lg font-bold text-neutral-900">Finaliza tu compra</h2>
          </section>
          ...
          {/* Green badge: price available */}
          <section className="px-4 pt-3.5">
            <div className="flex items-center gap-3 rounded-xl bg-[#E6F5EC] px-4 py-3">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#0B7B44" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6L9 17l-5-5" /></svg>
              <span className="text-sm font-semibold text-[#0B7B44]">Precio disponible</span>
            </div>
          </section>
          ...
          <section className="px-4 pt-3.5">
            <div className="rounded-[18px] border border-black/[0.08] bg-white p-4">
              <div className="text-xs font-semibold text-neutral-500">Precio por unidad</div>
              <div className="mt-1 text-[28px] font-extrabold leading-none tracking-tight text-neutral-900">{eur(displayPricePerUnit)}</div>
              <div className="mt-2 flex items-center gap-2">
                {group.pvp > displayPricePerUnit && (
                  <span className="text-sm text-neutral-400 line-through">{eur(group.pvp)}</span>
                )}
                {savingsPerUnit > 0.01 && (
                  <span className="inline-flex items-center rounded-lg bg-[#E6F5EC] px-2.5 py-1 text-xs font-bold text-[#0B7B44]">
                    Ahorras {fmtSaving(savingsPerUnit * quantity)}
                  </span>
                )}
              </div>
              ...
              <div className="flex items-baseline justify-between">
                <span className="text-sm font-semibold text-neutral-700">Total a pagar hoy</span>
                <span className="text-lg font-bold text-neutral-900">{eur(displayTotal)}</span>
              </div>
            </div>
          </section>

          {/* Trust card 1: Price refund guarantee */}
          <section className="px-4 pt-3.5">
            <div className="flex gap-3 rounded-[18px] border border-[#CFEADA] bg-[#EEF8F1] p-[13px]">
              <div className="grid h-[34px] w-[34px] flex-none place-items-center rounded-[10px] bg-[#0B7B44]">
              ...
              <div>
                <div className="text-[15px] font-bold text-neutral-900">Garantía de reembolso de precio</div>
                <div className="mt-0.5 text-xs leading-[1.5] text-neutral-500">Si el precio baja antes del cierre, pagarás el precio más bajo automáticamente.</div>
              </div>
            </div>
          </section>

          {/* Trust card 2: Secure purchase */}
          <section className="px-4 pt-3.5">
            <div className="flex gap-3 rounded-[18px] border border-black/[0.08] bg-white p-[13px]">
              <div className="grid h-[34px] w-[34px] flex-none place-items-center rounded-[10px] bg-brand">
                <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg>
              </div>
              <div>
                <div className="text-[15px] font-bold text-neutral-900">Compra 100% segura</div>
                <div className="mt-0.5 text-xs leading-[1.5] text-neutral-500">Pago protegido con 3D Secure. Tu dinero está asegurado en todo momento.</div>
              </div>
            </div>
          </section>
        </>
      ...
      <Elements stripe={stripePromise} options={elementsOptions}>
        <InnerForm
          ...
          /* UX-14 . El importe que de verdad se bloquea en la tarjeta. Sale de
             `effectiveAmount`, el mismo que viaja a Stripe, asi que no puede
...
/* Logos de metodo de pago (sin assets externos, inline). */
/**
 * UX-04 . Fila de metodos aceptados, bajo el boton de pago.
 *
 * Llevaba tambien Apple Pay y Google Pay. Se han retirado por dos razones
 * independientes, y basta con una:
 *
 *  1. Es marcado ESTATICO: no pregunta nada al Payment Element. Un usuario de
 *     Android veia Apple Pay, que en su telefono es imposible; uno de Safari
 *     veia Google Pay; y cualquiera sin tarjeta guardada no veia ninguno de los
 *     dos al llegar al pago. Prometia en el punto de pago algo que en la mitad
 *     de los casos no aparece.
 *  2. Las carteras ademas exigen registrar el dominio en Stripe
 *     (Settings -> Payments -> Payment method domains). En live la lista estaba
 *     VACIA (comprobado el 13-sep-2026); en test es UNKNOWN, la clave de la
 *     integracion no tiene permiso para consultarlo.
 *
 * Las marcas de tarjeta si se quedan: el Payment Element siempre ofrece tarjeta,
 * asi que VISA y Mastercard son ciertas en todos los casos.
 *
 * PARA DEVOLVERLAS: registrar el dominio en Stripe (test y live son listas
 * separadas), comprobar en un movil real que la cartera aparece de verdad en el
 * Element, y solo entonces volver a anadir los dos pills aqui.
 */
function PayLogos() {
 ...
 *
 * Hasta ahora el comprador firmaba una autorizacion de pago sin que existiera en
 * ninguna pantalla un enlace a las condiciones bajo las que firma. La LSSI pide acceso
 * permanente a esa informacion y la normativa de consumo pide informacion
 ...
 *
 * Los textos siguen siendo un borrador pendiente de revision juridica: esto resuelve
...
/* Fila "Como funciona Gropo?" -- abre el bottom sheet compartido. */
...
function InnerForm({
   ...
   *  sera el precio real y la diferencia se libera al capturar. */
  holdTotal: number;
  savings: number;
  showAdjust: boolean;
  joinMode?: 'comprar' | 'esperar';
  ...
  /** Esperador que todavia no ha visto su objetivo alcanzado: el importe no es
   *  un total a pagar, es un techo condicionado a que el grupo llegue. */
  const esperando = visualMode === 'esperar' && !targetReached;
  /** Solo pasa con un esperador cuyo objetivo YA se alcanzo: el hold es su
   *  objetivo y el cobro sera el precio real, mas bajo. */
   ...
   *
   * Antes habia un unico booleano para los ocho campos obligatorios y un solo
   * mensaje --"Completa todos los campos antes de continuar"-- que ademas se pintaba
   * DESPUES de la seccion "3. Pago seguro", mientras el submit hacia scroll hasta
   * arriba. Es decir: la pantalla saltaba al principio y el aviso se quedaba fuera
   * de vista, detras de tres secciones y del footer fijo. El comprador veia que algo
   * se movia y nada mas.
   *
   * `error` sigue existiendo para lo que si es global: fallos de Stripe, de red y
  ...
  // Precarga: si el usuario ya compro antes, no debe volver a teclear sus datos
  // ni su direccion. La identidad vive en localStorage (la guarda este mismo
  // checkout al confirmar) y la direccion predeterminada en el perfil.
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setPrefilled(true);
      } catch { /* sin perfil: el formulario se queda vacio */ }
    })();
  ...
  async function handleSubmit() {
    ...
    // lo que el teclado del movil abre.
    // A-25 . El email lleva ademas comprobacion de formato: es la llave con la que se
    // recupera el pedido (`get_my_groups` exige telefono + email), y un fallo ahi no
    // da error en ninguna parte -- el dinero se retiene igual y el pedido desaparece
    // de /mis-grupos y /notificaciones. El servidor lo valida tambien, en
    ...
    // Recordar identidad (telefono + email) para la auto-carga de "Mis grupos".
  ...
  return (
    <>
      {/* -- TOTAL Y RETENCION (UX-14) --
          Antes aqui solo habia un "Subtotal" y ninguna linea de envio ni total,
          y el CTA decia "Hoy 0 EUR" sin contar que hay un importe bloqueado en la
          tarjeta. Ahora se dice lo que se paga, lo que se retiene, y en que se
      ...
      <section className="px-4 pt-6">
        <div className="rounded-2xl border border-neutral-100 bg-white p-3.5">
          ...
          {/* A-14 . El ahorro SI se calculaba, pero estaba al final del bloque, en
              12 px y despues del parrafo legal -- mientras en la home es un badge verde
              en cada tarjeta. En las gafas son 89 EUR frente a 197 EUR: un 55 %. El
              argumento economico mas fuerte del producto se desvanecia justo en la
      ...
      <section ref={datosRef as any} className={SECTION}>
        <h2 className="text-base font-bold text-neutral-900">Datos de contacto</h2>
        <p className="mt-0.5 mb-3 text-xs text-neutral-500">Tu información de contacto</p>
      ...
      <section className={SECTION}>
        <h2 className="text-base font-bold text-neutral-900">Dirección de envío</h2>
        <p className="mt-0.5 mb-3 text-xs text-neutral-500">¿Dónde enviamos tu pedido?</p>
        {prefilled && (
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          <Field id="postal_code" label="Código postal" inputMode="numeric" placeholder="Código postal"
            ...
            value={s.postal_code} onChange={(e) => {
              ...
              // Ciudad: buscar por API cuando el CP tiene 5 digitos
              if (cp.length === 5) {
      ...
      {/* -- 3. METODO DE PAGO -- */}
      <section className={SECTION}>
        <h2 className="text-base font-bold text-neutral-900">Método de pago</h2>
        <p className="mt-0.5 mb-3 text-xs text-neutral-500">Pago seguro con encriptación SSL</p>
      ...
      {/* -- FOOTER FIJO: CTA -- */}
      {/* A-32 . En movil sigue fija al borde inferior. En escritorio, fijarla al
          fondo de una pantalla de 1.440 la dejaba a medio metro del formulario: ahi
          pasa a ser pegajosa DENTRO de su columna, siempre a la vista y junto a los
          campos que se estan rellenando. */}
          ...
          >
            {confirming ? (
              ...
              <>
                ...
                {noStock ? 'Sin unidades disponibles' : visualMode === 'comprar' ? `Comprar ahora · ${eur(total)}` : 'Unirme al grupo · 0€ hoy'}
              </>
            )}
          </button>
          {visualMode === 'comprar' && (
            <p className="mt-2 text-center text-xs text-neutral-500">
              Retenemos el importe hoy. Si el precio baja, pagarás menos. Si el grupo no se completa, no se te cobra nada.
            </p>
          )}
          {visualMode === 'esperar' && (
            <p className="mt-2 text-center text-xs text-neutral-500">
              Solo pagas si el grupo alcanza tu precio objetivo. Si no, se cancela sin coste.
            </p>
          )}

          {/* -- TRUST BADGES: validacion institucional en el punto de friccion -- */}
