...
function fmtWa(n: number) {
  return n.toFixed(2).replace('.', ',')
}

export default function ShareButton({ productName, bestPrice, pvp, nextPrice, groupId }: Props) {
  ...
  return (
    <button
      onClick={handleShare}
      className="w-full flex items-center justify-center gap-2 border-2 border-gray-200 text-gray-700 font-semibold text-sm py-3.5 rounded-2xl hover:border-brand hover:text-brand active:scale-[0.98] transition-all"
    >
